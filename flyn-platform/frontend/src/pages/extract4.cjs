...
while (true) {
  ...
  console.log(`Checking ${name} at idx ${idx}`);
  
  let arrowIdx = content.indexOf('=>', panelIdx);
  if (arrowIdx === -1 || arrowIdx > panelIdx + 150) {
    console.log(`  -> no arrow found near panelIdx`);
    idx += searchStr.length;
    continue;
  }
  
  let bodyStartIdx = content.indexOf('{', arrowIdx);
  if (bodyStartIdx === -1 || bodyStartIdx > arrowIdx + 50) {
  ...
  if (endIndex !== -1) {
    ...
    idx = endIndex + 1;
    console.log(`  -> Success! Moved idx to ${idx}`);
  } else {
    console.log(`  -> Failed to find end brace`);
    idx += searchStr.length;
  }
}
